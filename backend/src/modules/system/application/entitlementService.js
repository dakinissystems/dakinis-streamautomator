...
import { Entitlement } from '../infrastructure/models.js';
...
export async function getUserEntitlements(userId) {
  try {
    ...
    if (!user) return DEFAULT_ENTITLEMENTS.none;
    ...
    logger.error('Error getting user entitlements', { userId, error: error.message });
...
export async function syncEntitlementsFromLicense(userId, licenseType, licenseExpiresAt) {
  try {
    ...
    logger.error('Error syncing entitlements', { userId, licenseType, error: error.message });
  }
}
...
export async function setEntitlement(userId, feature, value, expiresAt = null) {
  try {
    ...
    logger.info('Entitlement override set', { userId, feature, value });
  } catch (error) {
    logger.error('Error setting entitlement override', { userId, feature, error: error.message });
    throw error;
  }
}
