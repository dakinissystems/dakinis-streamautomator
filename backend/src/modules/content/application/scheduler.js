...
import { Integration } from '../../integrations/infrastructure/models.js';
...
import { checkIdempotency, markPublicationAttempted } from '../../system/application/idempotencyService.js';
import { canPublish, recordPublication } from '../../system/application/rateLimitService.js';
...
async function publishToPlatform(content, platform) {
  ...
  try {
    ...
    if (platform === 'twitter') {
      ...
      const normalizedPlatforms = (content.platforms || []).map((p) => (p || '').trim().toLowerCase());
      ...
      const { TwitchService } = await import('../../integrations/application/twitchService.js');
    ...
    try {
      const { notifyContentFailed } = await import('../../../services/websocketService.js');
...
export async function publishContent(content) {
  ...
  let hasQueued = false;
  const normalizedPlatforms = (platforms || []).map((p) => (p || '').trim().toLowerCase());
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      else if (result.queued) hasQueued = true;
  ...
  const successfulPlatforms = allPlatforms.filter((p) => results[p]?.success === true && !results[p]?.failed && !results[p]?.skipped);
  const failedPlatforms = allPlatforms.filter((p) => results[p]?.failed === true || (results[p]?.success === false && !results[p]?.queued));
  ...
  if (hasQueued) {
    ...
    try {
      const { notifyContentPublished } = await import('../../../services/websocketService.js');
...
async function runTick() {
  try {
    ...
    for (let i = 0; i < due.length; i += concurrency) {
      ...
      await Promise.allSettled(batch.map((content) => publishContent(content)));
    }
  } catch (err) {
