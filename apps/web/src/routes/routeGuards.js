  ...
  if (!user.isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-purple-100">
        ...
        <button type="button" onClick={() => navigate('/dashboard')} className="px-6 py-2 bg-blue-600 text-white rounded shadow hover:bg-blue-700 transition">{t('common.goToUserDashboard') || 'Go to User Dashboard'}</button>
      </div>
    );
  }
  return children;
  ...
  if (user.isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-purple-50 to-blue-100">
        ...
        <button type="button" onClick={() => navigate('/admin')} className="px-6 py-2 bg-purple-600 text-white rounded shadow hover:bg-purple-700 transition">{t('common.goToAdminDashboard') || 'Go to Admin Dashboard'}</button>
      </div>
    );
  }
  return children;
