...
export class TwitchService {
  ...
  async makeRequest(endpoint, userAccessToken = null, options = {}) {
    ...
    if (!token) throw new Error('Twitch access token not available');
  ...
  async getBroadcasterId(userAccessToken) {
    ...
    if (!user?.id) throw new Error('Could not get Twitch user id');
    return user.id;
  }
  ...
  async getSubscriptions(broadcasterId, userAccessToken) {
    try {
      ...
      if (error.response?.status === 403) {
        logger.warn('Twitch subscriptions scope not available', { broadcasterId });
  ...
  async getBitsLeaderboard(broadcasterId, userAccessToken, period = 'all') {
    try {
      ...
      return { total: totalBits, leaderboard };
    } catch (error) {
      if (error.response?.status === 403) {
  ...
  async getStreamByUserId(broadcasterId) {
    ...
    try {
      const data = await this.makeRequest(`/streams?user_id=${encodeURIComponent(broadcasterId)}`);
