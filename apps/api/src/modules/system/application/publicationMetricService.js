...
import { PublicationMetric } from '../infrastructure/models.js';
import { User } from '../../users/infrastructure/models.js';
import { Media } from '../../content/infrastructure/models.js';
...
export async function getCostMetricsForAdmin() {
  ...
  const totalJobs = Object.values(metricsByUser).reduce((sum, m) => sum + (m.jobsExecuted || 0), 0);

  const byUser = users.map((u) => {
    const m = metricsByUser[u.id] || { jobsExecuted: 0, totalDurationMs: 0 };
    const jobsExecuted = m.jobsExecuted || 0;
    const avgExecutionMs = jobsExecuted > 0 ? Math.round(m.totalDurationMs / jobsExecuted) : null;
